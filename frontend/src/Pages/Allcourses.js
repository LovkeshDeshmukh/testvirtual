...
function Allcourses() {
    ...
    const courseGetData=()=>{
        axios.get(baseUrl + "addcourse"). then((res)=>setCourseList(res.data.data))
    }

    const [studentList, setStudentList]=useState([])
    const stdSkills = localStorage.getItem("stdSkills")
    const getStudentData=()=>{
        axios.get(baseUrl + "addstudents"). then((res)=>setStudentList(res.data.data.filter((i)=>i.type==="teacher")))
    }
                ...
                <div style={{height:"70vh", width:"100%", backgroundColor:"white",display:"flex",flexDirection:"column",alignItems:"center",paddingTop:"10px", overflowY:"auto"}}>
                    ...
                    <div style={{height:"12vh", width:"95%", backgroundColor:"whitesmoke",borderRadius:"10px", display:"flex",marginTop:10}}>
                        ...
                        <div style={{height:"12vh", width:"50%",display:"flex",flexDirection:"column",justifyContent:"center"}}>
                            <label style={{fontSize:"19px", fontWeight:"bold",letterSpacing:1,height:25}}>{i.courseName}</label>
                            <label>by {i.teacherName}</label>
                        </div>
                        <div style={{height:"12vh", width:"30%", display:"flex", justifyContent:"center",alignItems:"center"}}>
                            <button onClick={()=>{localStorage.setItem("courseData1", JSON.stringify(i)); navigate("/Viewcourses")}} style={{height:"40px", width:"80%", backgroundColor:"black", color:"white",borderRadius:10,border:"none"}}>Veiw courses</button>
                        </div>
                    </div>)}
                   ...
                   <div style={{height:"70vh", width:"45%", backgroundColor:"white",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"start",}}>
                    <label>Assigned to you</label>
                    
                    {studentList.filter((i)=>i.specialization===stdSkills).sort((a,b)=>a.experience<b.experience?1:-1).map((j)=>
                    <div style={{height:"12vh", width:"95%", backgroundColor:"whitesmoke",borderRadius:"10px", display:"flex", marginTop:"20px"}}>
                        <div style={{height:"12vh", width:"20%",display:"flex", justifyContent:"center", alignItems:"center"}}>
                            <div style={{height:"9vh", width:"70%",backgroundColor:"white",borderRadius:10,display:"flex", justifyContent:"center", alignItems:"center"}}>
                                <img style={{height:"5vh", width:"60%",}} src='https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/2300px-React-icon.svg.png'/>
                            </div>
                        ...
                        <div style={{height:"12vh", width:"50%",display:"flex",flexDirection:"column",justifyContent:"center"}}>
                            <label style={{fontSize:"19px", fontWeight:"bold",letterSpacing:1,height:25}}>{j.specialization}</label>
